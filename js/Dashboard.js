...
//Start Transformations
	...
	var time_session_ = ndx.dimension(function(d) { return d.Time_session; });
	...
	var projectByTimeSession = time_session_.group();
	...
	var totalfatalitiessession = time_session_.group().reduceSum(function(d) {
		return d.Total_Fatalities;
	});
...
console.log(maxDate);
	...
	var sessionFatalities = dc.barChart("#session-Fatalities");
  ...
  selectField = dc.selectMenu('#menuselect')
    ...
    maneuverFatalities
     ...
     sessionFatalities
        ...
        .height(220)
        .transitionDuration(1000)
        .dimension(time_session_)
        .ordinalColors(["#33d8ff"])
        .group(totalfatalitiessession)
        .margins({top: 10, right: 50, bottom: 30, left: 50})
        .centerBar(false)
        .gap(5)
        .elasticY(true)
        .x(d3.scale.ordinal().domain(time_session_))
        .xUnits(dc.units.ordinal)
        .renderHorizontalGridLines(true)
        .renderVerticalGridLines(true)
        .ordering(function(d){return d.value;})
...
dcCharts = [selectField, dateChart, collisionTypeChart, hitAndRunChart, policeStationChart, intersectionChart,maneuverFatalities,sessionFatalities];
